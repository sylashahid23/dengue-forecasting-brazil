...
for state in df["state"].unique():
    ...
    # Validation sequences: use the final 12 training months as context
    val_input = pd.concat(
        [
    ...
    # Test sequences: use the final 12 validation months as context
...
# ============================================================
# Train three-model stacking meta-model
# ============================================================

print()
print("=" * 60)
print("Training Three-Model Stacking Meta-Model")
print("=" * 60)
...
print()
print("=" * 60)
print("LSTM + Transformer + DLinear Stacking Results")
print("=" * 60)
...
output.to_csv(
    OUTPUT_DIR /
    "all_three_stacking_predictions.csv",
    index=False
)
...
metrics_output = pd.DataFrame(
    {
        "Model": [
            "LSTM + Transformer + DLinear Stacking"
        ],
        "RMSE": [rmse],
        "MAE": [mae]
    }
...
metrics_output.to_csv(
    OUTPUT_DIR /
    "all_three_stacking_metrics.csv",
    index=False
)
...
state_results_df.to_csv(
    OUTPUT_DIR /
    "all_three_stacking_state_metrics.csv",
    index=False
)
...
coefficients_output.to_csv(
    OUTPUT_DIR /
    "all_three_stacking_coefficients.csv",
    index=False
)
