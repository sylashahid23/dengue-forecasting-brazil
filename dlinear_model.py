...
class DLinearModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_seasonal = nn.Linear(SEQUENCE_LENGTH, 1)
        self.linear_trend = nn.Linear(SEQUENCE_LENGTH, 1)
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(
            model.state_dict(),
            "saved_models/best_dlinear_model.pth"
        )
...
model.load_state_dict(
    torch.load("saved_models/best_dlinear_model.pth")
)
...
results_df.to_csv(
    "results/dlinear_predictions.csv",
    index=False
)
...
for state in results_df["state"].unique():
    ...
    plt.title(f"DLinear Predictions - {state}")
    plt.xlabel("Month")
    ...
    plt.savefig(
        f"results/dlinear_predictions_{state}.png"
    )
...
plt.savefig(
    "results/dlinear_predictions.png",
    dpi=300
)
